...
- [x] 1.4 Scaffold `frontend/` Angular workspace (standalone + signals default), verify `ng build` passes
...
- [x] 5.1 Validator tests for every field rule (name lengths, title required, ISBN format/checksum, year 1450..current, pagination bounds, sort whitelist)
- [x] 5.2 Business-rule branch tests: duplicate-name detection, delete-in-use detection (EF semantics-free only, per design.md D7)

## 6. Integration tests (Testcontainers)

- [x] 6.1 Test fixture: Testcontainers PostgreSQL + WebApplicationFactory, migrations applied, per-class data isolation
- [x] 6.2 CRUD happy-path scenarios for all three entities against the real database
- [x] 6.3 Error-contract scenarios: 400 shape with `errors`, 404, 409 (duplicate name, duplicate ISBN, delete-in-use), pagination metadata and out-of-range rejection, sort whitelist rejection

## 7. Containerization

- [x] 7.1 Multi-stage API Dockerfile (sdk build → aspnet runtime), migrations applied at startup in Development
- [x] 7.2 Frontend Dockerfile (node build → nginx) with `/api` reverse proxy to the api service
- [x] 7.3 `docker-compose.yml`: db (healthcheck) → api → web dependency chain; verify cold `docker compose up --build` yields a working system

## 8. Angular SPA (library-spa spec)

- [x] 8.1 Core: typed API client services, HTTP interceptor mapping ProblemDetails (400→form controls, 409/404/500→banner), app shell with section navigation
- [x] 8.2 Genres feature: list (search + pagination), create/edit reactive form with server-error writeback, delete with confirmation
- [x] 8.3 Authors feature: same shape as genres
- [x] 8.4 Books feature: list showing author/genre names, form with catalog-driven author/genre selectors, delete with confirmation
- [x] 8.5 Loading indicators, empty states, and in-use delete message per library-spa spec; final pass against every library-spa scenario

## 9. Documentation

- [x] 9.1 ADRs 0001–0004 in `docs/adr/` per design.md D9
- [x] 9.2 README: solution overview, architecture, backend/frontend organization, database choice, trade-offs, testing strategy, known limitations, with-more-time improvements, and one-command run instructions
- [x] 9.3 Verify README instructions end-to-end from a clean clone (fresh terminal, `docker compose up --build`, run test suites)

## 10. Final verification

- [x] 10.1 `dotnet test` green (unit + integration), `ng build` green, `openspec validate bootstrap-library-management --strict` clean
- [x] 10.2 Walk every spec scenario (genre-management, author-management, book-management, api-contract, library-spa) and confirm observable pass
